...
Retained from v8.1:
  --coverage-as-tiebreaker  Disable hard coverage gate (oral / low-cov datasets)
  --min-checkm2-bp          Minimum total bin size for CheckM2 input list
  --min-af                  Minimum skani alignment fraction to retain a pair
...
# ── Constants ────────────────────────────────────────────────────────────────
MIN_LEN = 5000          # bp — min contig length for main/secondary clustering
...
LEIDEN_RES_MAIN = 1.3       # finer splits for closely related oral taxa
LEIDEN_RES_SECONDARY = 0.8  # coarser; appropriate for above-genus groups

TNF_GATE_MAIN = 0.85        # minimum TNF cosine similarity for hybrid gate in Tiers 1/2
...
def run_checkm2(bin_list_path: str, outdir: str, threads: int) -> pd.DataFrame:
    ...
    # CheckM2 Name column may be the bare stem ("cluster_0004_bin") or include
    # the extension ("cluster_0004_bin.fasta") depending on version — handle both.
#    def _parse_cluster_name(name: str) -> str:
#        stem = os.path.splitext(str(name))[0]   # strip .fasta if present
#        base = os.path.basename(stem)           # remove path
#        return base        # strip _bin suffix
#
#    qdf["cluster"] = qdf["Name"].apply(_parse_cluster_name)
#
#    # Warn if any derived cluster IDs are duplicated — would break the join
#    dupes = qdf["cluster"][qdf["cluster"].duplicated()].tolist()
#    if dupes:
#        log.warning(f"Duplicate cluster IDs in CheckM2 output: {dupes[:10]} — keeping first occurrence")
#        qdf = qdf.drop_duplicates(subset="cluster", keep="first")
...
def run_drep(
    ...
    for _, row in passing.iterrows():
        cid = row["cluster"]
        bin_path = os.path.join(clusters_dir, "bins", cid + "_bin.fasta")
...
def merge_checkm2_into_summary(summary_path: str, checkm2_df: pd.DataFrame) -> None:
    if checkm2_df.empty:
        log.warning("CheckM2 DataFrame is empty — skipping summary merge")
        return

    summary = pd.read_csv(summary_path, sep="\t", low_memory=False)

    # 1. Normalize CheckM2 column names (handles version differences)
    checkm2_df.columns = [c.lower() for c in checkm2_df.columns]
    col_rename = {
        "completeness_general": "Completeness_General",
        "contamination": "Contamination",
        "completeness_model_used": "quality", # Fallback if 'quality' isn't literal
        "name": "cluster",
    }
    # If a column named 'quality' already exists, keep it; otherwise use model field
    if "quality" in checkm2_df.columns:
        col_rename["quality"] = "quality"
        
    checkm2_df = checkm2_df.rename(columns=col_rename)

    # 2. Derive 'cluster' ID from 'name' (Crucial fix for path-based lists)
    # This handles "outdir/clusters/bins/cluster_0001_bin.fasta" -> "cluster_0001"
    #checkm2_df["cluster"] = checkm2_df["name"].apply(
    #    lambda x: os.path.basename(str(x)).replace(".fasta", "")
    #)

    # 3. Create mapping and diagnostic check
    qmap = checkm2_df.set_index("cluster")
    summary_clusters = set(summary["cluster"].unique()) - {"unassigned"}
    checkm2_clusters = set(qmap.index)
    
    matched = summary_clusters & checkm2_clusters
    missing_from_checkm2 = len(summary_clusters) - len(matched)
    
    log.info(
        f"CheckM2 merge: {len(matched):,}/{len(summary_clusters):,} clusters matched. "
        f"({missing_from_checkm2:,} clusters in summary were not assessed by CheckM2, "
        f"likely below the --min-checkm2-bp threshold)"
    )

    # 4. Map values back to summary
    summary["checkm2_completeness"] = summary["cluster"].map(qmap["Completeness_General"])
    summary["checkm2_contamination"] = summary["cluster"].map(qmap["Contamination"])
    
    # Check if 'quality' column exists in qmap before mapping
    if "quality" in qmap.columns:
        summary["checkm2_quality"] = summary["cluster"].map(qmap["quality"]).fillna("not_assessed")
    else:
        summary["checkm2_quality"] = "not_assessed"

    summary.to_csv(summary_path, sep="\t", index=False)
    log.info(f"Updated {os.path.basename(summary_path)} with CheckM2 quality columns")
...
def main() -> None:
    ...
    parser.add_argument("--leiden-res-main", type=float, default=LEIDEN_RES_MAIN,
                        help="Leiden resolution for tier-1 (genus/species) clustering. "
                             "Higher = finer splits. Raise to 1.5+ for dense oral datasets.")
    ...
    parser.add_argument("--leiden-res-t4", type=float, default=0.8,
    ...
    parser.add_argument("--cov-recovery-tnf-min", type=float, default=0.90,
                        help="Minimum TNF cosine similarity for tier-5 recovery. "
                             "Set to 0 to disable the TNF guard (more permissive).")
    parser.add_argument("--min-checkm2-bp", type=int, default=200_000,
    ...
    if not args.skip_checkm2:
        ...
        merge_checkm2_into_summary(
            os.path.join(args.outdir, "cluster_summary.tsv"), checkm2_df
        )
    else:
